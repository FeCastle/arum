...
/*  1b. Create process, get BPatch_image object                           */
    BPatch_process * proc =  bpatch.processCreate( argv[1] , 
                                                   (const char**)(argv + 1) );
    assert ( proc ) ;
...
/*  3d. Find the entry point of main                                      */
    std::vector< BPatch_point * > *mainEntryPoints = 
        main->findPoint( BPatch_entry );
    BPatch_point *mainEntry = (*mainEntryPoints)[0];

/*  3e. Insert snippet at entry point of main                             */
    proc->insertSnippet( initSnippet, 
                         *mainEntry, 
                         BPatch_firstSnippet );
...
/*  4d. Find the entry point of malloc                                    */
    std::vector< BPatch_point * > *mallocEntryPoints = 
        bp_malloc->findPoint( BPatch_entry );
    BPatch_point *mallocEntry = (*mallocEntryPoints)[0];

/*  4d. Insert snippet at entry point of malloc                           */
    proc->insertSnippet( conditionalAssignment, 
                         *mallocEntry, 
                         BPatch_firstSnippet );
